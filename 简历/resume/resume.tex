...
\begin{itemize}
  \item 完成了SVM,XGBoost,RandomForest,LightGBM基于if-idf,共现矩阵,词向量等文本特征的文本分类;
  \item 完成了基于不同的深度学习特征提取器(CNN,RNN,transformer)的文本分类;
  \item 完成了基于attention,capsule,max-pooling,average-pooling等高层特征提取器的文本分类;
  \item 完成了基于ELMO预训练词向量文本特征的文本分类;
  \item 完成了基于Focal loss,改变阀值,样本采样等处理类别不平衡方法的文本分类;
  \item 完成了基于stacking策略的模型ensemble;
  \item 完成了基于Bert模型的长文本分类问题,分析了微调方法,长文本的处理等对分类结果的影响.
\end{itemize}
...
\begin{itemize}[parsep=0.5ex]
  \item Python\ \ Tensorflow \ \ Linux\ \ Scikit-learn \ \ Ipython\ Notebook \ \  Git\ \ VsCode \ \ Markdown \ \ Microsoft Office
\end{itemize}
